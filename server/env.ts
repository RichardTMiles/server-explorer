...
export const config = {
  ...
  switchHost: process.env.SWITCH_HOST?.trim() || "192.168.1.193",
  switchLabel: process.env.SWITCH_LABEL?.trim() || "HP ProCurve 2810-24G",
  snmpCommunity: process.env.SNMP_COMMUNITY?.trim() || undefined,
  allowSwitchWriteCommands: readBoolean(process.env.ALLOW_SWITCH_WRITE_COMMANDS ?? process.env.ALLOW_WRITE_COMMANDS, false),
